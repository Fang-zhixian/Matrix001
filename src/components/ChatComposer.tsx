...
export default function ChatComposer({
  ...
  useEffect(() => {
    syncTextareaHeight(
      ...
      readyMode ? READY_COMPOSER_MIN_HEIGHT : COMPACT_COMPOSER_MIN_HEIGHT
    );
  }, [readyMode, value]);
  ...
  useEffect(() => {
    syncTextareaHeight(
      ...
      EXPANDED_COMPOSER_MAX_HEIGHT,
      EXPANDED_COMPOSER_MIN_HEIGHT
    );
  }, [isExpanded, value]);
    ...
    <AnimatePresence>
      {isModelMenuOpen ? (
        <motion.div
          ...
          className={`absolute z-20 w-[360px] rounded-[1.2rem] border border-[#e6e8ee] bg-[#fcfcfd] p-2.5 shadow-[0_18px_48px_rgba(36,39,46,0.10)] ${
            mode === 'expanded'
        ...
        >
          <div className="px-2.5 pb-2 text-[12px] font-medium text-slate-400">选择模型</div>
          <div className="max-h-[min(42vh,360px)] space-y-0.5 overflow-y-auto pr-1">
            {modelMenuItems.map(({ provider, model, isConfigured }) => (
              <button
                ...
                className={`flex w-full items-center justify-between rounded-[0.95rem] px-2.5 py-2.5 text-left transition ${
                  isConfigured
              ...
              >
                <div className="flex min-w-0 items-center gap-2.5">
                  <ProviderMark providerId={provider.id} size="sm" muted={!isConfigured} />
                  <div className="min-w-0">
                    <div className={`truncate text-[14px] font-medium ${isConfigured ? 'text-slate-900' : 'text-slate-400'}`}>
                      {model.label}
                    </div>
                ...
                <div className="ml-3 flex items-center gap-2">
                  {!isConfigured ? <Lock className="h-3.5 w-3.5 text-slate-300" /> : null}
                  {selectedProviderId === provider.id && selectedModel === model.id ? (
                    <Check className="h-[18px] w-[18px] text-slate-900" />
                  ) : null}
                </div>
              </button>
            ))}
          ...
          {!modelMenuItems.some((item) => item.isConfigured) ? (
            <div className="px-2.5 pt-3 text-[11px] text-slate-400">
  ...
  const renderAttachments = (size: 'compact' | 'expanded') => {
    ...
    const cardSize = isExpandedMode ? 'h-[88px] w-[88px]' : readyMode ? 'h-[58px] w-[58px]' : 'h-[64px] w-[64px]';
    ...
    return (
      <div
        className={`flex items-start overflow-x-auto pb-1 ${
          isExpandedMode ? 'mb-5 gap-3' : readyMode ? 'mb-2.5 gap-2' : 'mb-3 gap-2.5'
        }`}
      >
  ...
  const renderFooter = (mode: 'compact' | 'expanded') => {
    ...
    const isReadyCompact = readyMode && !isExpandedMode;

    return (
      <div
        className={`flex items-center justify-between ${
          isExpandedMode ? 'mt-5 gap-4' : isReadyCompact ? 'mt-2.5 gap-3' : 'mt-3.5 gap-4'
        }`}
      >
        <div className={`flex items-center ${isReadyCompact ? 'gap-2.5' : 'gap-3'}`}>
          <button
            onClick={triggerFilePicker}
            className={`inline-flex items-center justify-center rounded-full bg-black/[0.032] text-slate-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.98)] transition hover:bg-black/[0.05] ${
              isExpandedMode ? 'h-11 w-11' : isReadyCompact ? 'h-9 w-9' : 'h-10 w-10'
            }`}
            type="button"
            aria-label="上传文件"
          >
          ...
          <div className="relative">
            <button
              ...
              className={`inline-flex items-center gap-3 rounded-full bg-black/[0.032] text-slate-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.98)] transition hover:bg-black/[0.05] ${
                isExpandedMode ? 'px-4 py-2.5' : isReadyCompact ? 'px-3 py-2' : 'px-3.5 py-2.5'
              }`}
              type="button"
            >
              <ProviderMark providerId={selectedProvider.id} size="sm" />
              <div className="flex flex-col items-start leading-none">
                <span className={`${isReadyCompact ? 'text-[13px]' : 'text-[14px]'} font-semibold text-slate-800`}>
                  {selectedModelOption?.label ?? '选择模型'}
                </span>
                <span
                  className={`mt-1 uppercase tracking-[0.14em] text-slate-400 ${
                    isReadyCompact ? 'text-[9px]' : 'text-[10px]'
                  }`}
                >
        ...
        <button
          ...
          className={`rounded-full bg-slate-900 text-white shadow-[0_10px_22px_rgba(36,39,46,0.18)] transition-all active:scale-90 hover:bg-[#0071e3] disabled:scale-95 disabled:opacity-25 ${
            isExpandedMode ? 'h-12 w-12' : isReadyCompact ? 'h-10 w-10' : 'h-11 w-11'
          }`}
          type="button"
        >
          {isCurrentCanvasProcessing ? (
      ...
      <div ref={composerMenuRef}>
        <div
          className={`absolute left-1/2 z-50 w-full px-8 transition-all duration-300 ${
            readyMode
              ? 'top-[46%] max-w-[48rem] -translate-x-1/2 -translate-y-1/2'
          ...
          <div className="relative">
            ...
            <motion.div
              ...
              className={`relative border border-white/92 bg-white/48 backdrop-blur-[36px] shadow-[0_12px_34px_rgba(36,39,46,0.07)] transition-all hover:bg-white/54 ${
                readyMode
                  ? 'rounded-[1.7rem] px-[18px] pt-[14px] pb-[14px]'
            ...
            >
              ...
              <div
                className={`flex items-center justify-between ${
                  readyMode ? 'mb-2.5' : 'mb-3'
                }`}
              >
                <button
                  onClick={() => setIsExpanded(true)}
                  className={`inline-flex items-center justify-center rounded-full bg-black/[0.032] text-slate-500 shadow-[inset_0_1px_0_rgba(255,255,255,0.98)] transition hover:bg-black/[0.05] hover:text-slate-700 ${
                    readyMode ? 'h-8 w-8' : 'h-9 w-9'
                  }`}
                  type="button"
                  aria-label="放大输入框"
                >
              ...
              <textarea
                ...
                className={`w-full resize-none border-none bg-transparent text-slate-800 placeholder-slate-300 outline-none focus:outline-none focus:ring-0 ${
                  readyMode
                    ? 'text-[15px] leading-6 tracking-[-0.018em]'
